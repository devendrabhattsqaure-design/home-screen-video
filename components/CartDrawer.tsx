...
export default function CartDrawer() {
  ...
  const handleCompleteOrder = () => {
    ...
    const total = subtotal.toLocaleString("en-IN")
    const message = `🛍️ *New Order from Sacred Samagri*%0A%0A${orderDetails}%0A%0A*Subtotal: ₹${total}*%0A%0APlease confirm my order. Thank you! 🙏`

    // Clear cart and close drawer after sparkler effect
    setTimeout(() => {
      ...
      // Open WhatsApp
      window.open(
        `https://wa.me/918840403939?text=${message}`,
        "_blank"
      )
    }, 1000)
  }
          ...
          {items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Add some divine items to get started
              </p>
              <Link
                href="/products"
                onClick={closeCart}
        ...
        {items.length > 0 && (
          <div className="absolute bottom-0 left-0 right-0 border-t border-border bg-card p-6">
            ...
            >
              ...
              {isCompletingOrder ? "Processing..." : "Complete Order on WhatsApp"}
            </button>
          </div>
        )}
      </div>
